...
class DashboardAPICaller {
   static func dashboardAPICaller(CompletionHandler: @escaping(_ result: Result<DashboardResModel, NetworkError>) -> Void) {
        ...
        URLSession.shared.dataTask(with: request) { dataResponse, urlResponse, error in
            ...
            if error == nil ,
               ...
               let jsonResponse = try? JSONDecoder().decode(DashboardResModel.self, from: data) {
                print("My JSON Response for Dashboard API Caller is  ", jsonResponse)
