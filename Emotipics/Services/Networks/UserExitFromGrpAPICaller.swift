//
//  UserExitFromGrpAPICaller.swift
//  Emotipics
//
...
class UserExitFromGrpAPICaller {
    static func userExitFromGrpAPICaller(groupCode: String , CompletionHandler: @escaping(_ result: Result<GroupUserExitResModel, NetworkError>) -> Void){
        ...
        let urlString = baseURL + APIEndpoint.userExitFromGrp.rawValue
        ...
        URLSession.shared.dataTask(with: request) { dataResponse, urlResponse, error in
            ...
            if error == nil ,
               ...
               let jsonResponse = try? JSONDecoder().decode(GroupUserExitResModel.self, from: data) {
                print("JSON Response after user exit from group✅✅", jsonResponse)
