...
extension GroupListViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if notificationView {
            ...
            cell.noOfUsers.text = String((newResultArray[indexPath.row].members ?? 0) + 1) + " users"
            
//            let dateAndTime = compareDates(newResultArray[indexPath.row].datetime)
            if let currentDate = newResultArray[indexPath.row].datetime {
                
                cell.daysLbl.text = compareDates(currentDate)
            }
